...
|----------|---------------|------------|
| `authorize` / `auth` | Валидирует `client_id`, `redirect_uri`, `response_type`, `scope`, `access_type`, `prompt`. Клиент ищется **только среди клиентов провайдера `default`**. Далее: при живой сессии (cookie `SID` либо claim `sub` из `__Secure-AT`), `response_type=code` и `prompt`, не называющем экран, — запрашивает код у базы. Та отвечает `consent_required`, если пользователь ещё не дал этому клиенту согласия на запрошенные scope, и тогда браузер уходит на `oauth2.consent`; иначе код выдаётся и браузер перенаправляется на `redirect_uri?code=&state=`. При `prompt=consent` — сразу на `oauth2.consent`. Иначе — на страницу входа (`oauth2.identifier` или `oauth2.secret`). | `daemon.authorization_code(session, client_id, redirect_uri, scope, state, access_type, agent, host, consent)` (только при наличии сессии) |
...
| `identifier` | Извлекает `value` из тела запроса. Аутентификация: (1) заголовок `Authorization: Bearer`, (2) заголовки `Session`+`Secret`, (3) cookie `__Secure-AT`/`__Secure-SAT` по заголовку `X-Auth-Context`, либо (4) **ничего вовсе** — экраны входа и регистрации спрашивают это до того, как кто-либо вошёл, и модуль отвечает под собственным сервисным токеном (см. ниже). | `daemon.identifier(token, value)` |
...
| `consent` | Ответ на экран согласия. Требует **тела** запроса, `response_type` равного `code`, живой сессии и токена двойной отправки: cookie `__Host-CT` должна совпасть с полем `consent_token`. Записывает согласие, затем выдаёт код и перенаправляет на `redirect_uri?code=&state=`. | `daemon.authorization_code(…, consent := true)` |
...
### Сквозной вход и согласие

`GET /oauth2/authorize` выдаёт код авторизации сразу, если пользователь уже вошёл, —
без пароля и без повторного входа. Выдачу ограничивают два условия, и оба проверяются
в `daemon.authorization_code`, а не в этом модуле:

1. **Клиент должен принадлежать внутреннему провайдеру.** Запись в
   `conf/oauth2/google.json` — как и строка в `oauth2.audience` под внешним
   провайдером — регистрирует провайдера, чтобы проверять его маркеры. Клиентом
   данной установки она своего владельца не делает и кодов на локальных
   пользователей не даёт. На этот путь попадают только клиенты провайдера `default`.

2. **Пользователь должен дать этому клиенту согласие**, покрывающее запрошенные
   scope (`db.oauth2_consent`). Регистрация — не разрешение. Без действующего
   согласия база отвечает `consent_required`, и браузер уходит на экран согласия.

> **Изменение поведения (db-platform 1.2.11).** Раньше вошедший пользователь,
> попав на `/oauth2/authorize`, получал код, и ни о чём его не спрашивали; хватало
> любого `client_id`, присутствующего в `conf/oauth2/*.json` под любым провайдером.
> В существующих установках теперь один раз на клиента и пользователя показывается
> экран согласия — при условии, что сайт называет `oauth2.consent`. Там, где не
> называет, ветка вместо выдачи кода отвечает `consent_required` на `redirect_uri`
> клиента: закрыто, и это видно. См. «Где живёт экран согласия» ниже.

`POST /oauth2/consent` — это ответ на тот экран. Он записывает согласие и затем
выдаёт код ровно так же, как это сделала бы точка авторизации.

**Это POST, и он несёт токен двойной отправки.** Экран согласия минтит случайное
значение, хранит его в cookie с префиксом `__Host-` и атрибутом `SameSite=Strict` и
повторяет в теле формы; точка требует совпадения. Именно это отличает собственный
щелчок пользователя от чужой страницы, действующей от его имени: вошедший браузер
несёт свои cookie туда, куда его отправили, а `__Secure-AT` объявлена
`SameSite=None`, то есть уезжает и на межсайтовом POST.

> **Не заменяйте это проверкой `Origin`.** Она выглядит естественным заслоном и
> здесь слепа: рецепт развёртывания, с которым поставляются эти проекты,
> перезаписывает заголовок на каждом запросе `/oauth2/` —
> `location ^~ /oauth2/ { proxy_set_header Origin "https://$host"; }`, — и к моменту
> прихода запроса `Origin` читается как собственный, кто бы его ни отправил. Просто
> убрать перезапись тоже нельзя: на ней стоит выдача `client_secret` веб-клиенту в
> `POST /oauth2/token`.

Экран согласия и сервер авторизации должны отдаваться с **одного источника**. SPA,
поставляемое с модулем, исходит из этого (`VITE_API_HOST` пуст), и заслон на cookie
от этого зависит.

> **Сессионные cookie остаются `SameSite=None`, и это не небрежность.** Именно это
> делает сквозной вход возможным: приход на `/oauth2/authorize` с сайта клиента —
> межсайтовая навигация верхнего уровня, и при `SameSite=Strict` браузер сессию не
> отдаст, пользователь будет выглядеть невошедшим, и сценарий не запустится вовсе.
> Ужесточив `set_secure_cookies`, вы не укрепите эту точку — вы выключите
> возможность. `__Host-CT` может позволить себе `Strict` ровно потому, что минтится
> и тратится внутри одного источника.

### Где живёт экран согласия

Страницу называет ключ `oauth2.consent` в `conf/sites/*.json`:
...
**Без этого ключа ветка сквозного входа не работает.** Запасного пути нет намеренно:
модулю пришлось бы угадать URL на том хосте, куда пришёл запрос, а экран существует
только на хосте, отдающем SPA. Сайт без `oauth2.consent` отвечает `consent_required`
на `redirect_uri` клиента — это видно тому, кто интегрируется, в отличие от
перенаправления в 404.

Собственное SPA модуля отдаёт экран по пути `/authorize`.

### Развёртывание

**Патч базы и бинарь выкатываются вместе.** Половины не работают порознь, и каждый
отказ тих по-своему:

| Порядок | Что происходит |
|---------|----------------|
| Патч применён, бинарь ещё старый | `P00000011` удаляет восьмиаргументную `daemon.authorization_code`; старый бинарь зовёт её восемью аргументами и получает `function does not exist` → 500 на ветке входа |
| Новый бинарь, патч не применён | Нет таблицы `db.oauth2_consent`. Рутина это распознаёт и отвечает `server_error` с текстом *«The authorization server is not fully migrated»*, а номер патча пишет в журнал событий |
| Рутины обновлены без патча (`runme.sh --update` в одиночку) | То же самое. `--update` перезагружает рутины, но таблиц не создаёт; таблицу согласия и удаление старой сигнатуры делает патч |

Обратите внимание: база с новыми рутинами, но без патча, сохраняет **старую**
восьмиаргументную функцию, а она выдаёт коды никого не спрашивая. Обновление рутин
само по себе дыру не закрывает — её закрывает применение патча. После миграции
`\df daemon.authorization_code` обязана показывать ровно одну строку.

### Сервисный токен и почему браузер больше его не выписывает

`POST /oauth2/identifier` можно вызвать без учётных данных. Это не послабление:
экран, который его зовёт, — «этот адрес уже зарегистрирован?» — работает до того, как
кто-либо вошёл, и предъявить странице попросту нечего.

RFC 6749 §2.1 относит браузерное приложение к **публичным клиентам**, «incapable of
maintaining the confidentiality of their credentials», а §4.4 гласит: «the client
credentials grant type MUST only be used by confidential clients». То есть страница
не может держать секрет клиента, что бы она с ним ни делала: секрет, доехавший до
браузера, читается в инструментах разработчика, в бандле и во вкладке сети.
RFC 10017 (BCP 212) §6.1 называет замену: бэкенд, который «interacts with the
authorization server as a confidential OAuth client» от имени страницы.

Поэтому модуль держит собственный access-токен — `ServiceToken`, получаемый грантом
client credentials по клиенту `service` из `conf/oauth2` и обновляемый из
`heartbeat()`, — и вызывает с ним. Раньше токен выписывала себе сама страница, а это
означало, что сервер выдаёт сервисные учётные данные любому, кто попросит.

**Новых возможностей здесь не появляется** — эндпоинт и прежде был открыт любому,
кто сперва выпишет себе токен. Исчезают другое: цена пробы и оставляемый ею след.
Выписка токена писала строки в `db.session` и `db.token`, то есть злоупотребление
было видно и поддавалось счёту; анонимный POST не пишет ничего. Ограничение частоты
обращений теперь единственный контроль над перечислением идентификаторов — примените
рецепт ниже и к `/oauth2/identifier`, а не только к `/oauth2/consent`.

Требует записи `service` в `conf/oauth2/default.json`. Неаутентифицированные
вызывающие получают `503 temporarily_unavailable` всякий раз, когда токена нет: около
секунды после старта воркера или его перезапуска по SIGHUP, при недоступной базе и всё
время, пока обновление не удаётся. На аутентифицированных это не влияет.

Частично предъявленные учётные данные теперь игнорируются, а не отвергаются: заголовок
`Session` без `Secret` или `X-Auth-Context: service` без cookie `__Secure-SAT` раньше
давали 401, а теперь проваливаются в анонимный путь.

**Ограничьте частоту обращений.** `POST /oauth2/consent` анонимен и достижим без
сессии, а каждый отказ пишет строку в журнал. По обычному рецепту nginx маршрут
попадает под общий блок `^~ /oauth2/`, у которого своего предела, как правило, нет;
location с точным совпадением бьёт префиксный:
...
location = /oauth2/consent {
    ...
    # перезапись Origin из окружающего блока сохранить — на ней стоит
    # POST /oauth2/token; заслон согласия на неё намеренно не опирается.
}
```

### Про `redirect_uri`

`redirect_uri` сравнивается с зарегистрированными значениями клиента **точно**,
строка в строку. Это сделано намеренно — не «улучшайте» до сравнения по префиксу или
с нормализацией. Сравнение по префиксу превращает один зарегистрированный
`https://app.example.com/cb` в разрешение на `https://app.example.com/cb.attacker.com`,
а нормализация открывает всё семейство расхождений в разборе URL между проверяющим и
браузером.
...
AuthServer включает готовое **Vue 3 + Vite** одностраничное приложение для потоков аутентификации OAuth2. Оно находится в директории [`frontend/`](frontend/) и предоставляет:
...
> Полная инструкция по интеграции: [`frontend/README.md`](frontend/README.md) ([ru](frontend/README.ru-RU.md)).
