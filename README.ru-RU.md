...
> **`Origin` на этом сервере не является учётными данными.** Он что-то значит
> только когда его проставляет браузер: страница не может солгать о том, откуда
> она. Вызывающий, который браузером не является, пишет заголовок сам —
> `curl -H 'Origin: https://…'`, — а значение публично, это имя самого сайта.
> `POST /oauth2/token` подставляет секрет клиента `web` или `service` запросу с
> совпавшим Origin, и это безопасно лишь потому, что каждый доходящий туда грант
> требует второго секрета, которым вызывающий обязан уже владеть: пароля, кода
> авторизации, refresh-токена. У `client_credentials` такого второго секрета нет,
> поэтому без явного `client_secret` он отклоняется.
>
> **Не заменяйте проверкой `Origin` и заслон согласия.** Она выглядит естественным заслоном и
